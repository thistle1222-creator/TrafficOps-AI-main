...
type SimulatorResult = ReturnType<typeof computeRisk> & {
  predictedPriority: string;
  confidenceScore: number;
  modelUsed: string;
  message?: string;
...
<<<<<<< HEAD
  async function submitPrediction() {
    ...
    const crowdBoost = Math.min(20, Math.floor(crowd / 1000));
    baseline.score = Math.min(100, baseline.score + crowdBoost);
    baseline.factors.push({ label: `Crowd: ${crowd.toLocaleString()}`, value: crowdBoost });
    if (weather === "Heavy Rain") {
      baseline.score = Math.min(100, baseline.score + 10);
      baseline.factors.push({ label: "Weather: Heavy Rain", value: 10 });
    }

    try {
      const response = await fetch(`${API_BASE_URL}/ml/predict-priority`, {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        throw new Error(`ML API error ${response.status}: ${body}`);
      }
      ...
      setResult({
        ...
        predictedPriority: "Unknown",
        confidenceScore: 0,
        modelUsed: "fallback",
        message: "Using local fallback estimate.",
      });
    } finally {
      setRunning(false);
    }
...
<<<<<<< HEAD
            <select
              ...
              onChange={(e) => {
                ...
                setZone(nextZone);
                setJunction(JUNCTIONS[nextZone][0]);
              }}
              className="input"
            >
          ...
          <Field label="JUNCTION">
            <select value={junction} onChange={(e) => setJunction(e.target.value)} className="input">
...
<<<<<<< HEAD
                  <div className="mt-3 grid grid-cols-2 gap-2 text-[11px]">
                    <div className="hud-card p-2 bg-slate-950/70 border border-slate-800">
                      ...
                      <div className="mono text-sm mt-1">{result.predictedPriority}</div>
                    </div>
                    <div className="hud-card p-2 bg-slate-950/70 border border-slate-800">
                      <div className="mono text-[10px] text-muted-foreground">CONFIDENCE</div>
                      <div className="mono text-sm mt-1">{result.confidenceScore}%</div>
                    </div>
                    <div className="hud-card p-2 bg-slate-950/70 border border-slate-800">
                      <div className="mono text-[10px] text-muted-foreground">MODEL</div>
                      <div className="mono text-sm mt-1">{result.modelUsed}</div>
                    </div>
                    <div className="hud-card p-2 bg-slate-950/70 border border-slate-800">
                      <div className="mono text-[10px] text-muted-foreground">MESSAGE</div>
